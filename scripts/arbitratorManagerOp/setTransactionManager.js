...
async function main() {
    ...
    const transactionManager = await readConfig(networkName, "TRANSACTION_MANAGER");
    if (!transactionManager) {
        throw new Error("transactionManager address not found in config");
    }
    const tx = await arbitratorManager.connect(deployer).setTransactionManager(
        transactionManager,
    );
