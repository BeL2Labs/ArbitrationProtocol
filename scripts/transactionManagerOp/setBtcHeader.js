...
async function main() {
    ...
    const btcHeader = await readConfig(networkName, "BTC_HEADER_DATA");
    if (!btcHeader) {
        throw new Error("btcHeader address not found in config");
    }
    const tx = await transactionManager.connect(deployer).setBtcBlockHeaders(
        btcHeader,
    );
