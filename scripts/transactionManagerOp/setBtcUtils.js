...
async function main() {
    ...
    const newBtcUtils = await readConfig(networkName, "BTC_UTILS");
    if (!newBtcUtils) {
        throw new Error("newBtcUtils address not found in config");
    }
    const tx = await transactionManager.connect(deployer).setBtcUtils(
        newBtcUtils,
    );
